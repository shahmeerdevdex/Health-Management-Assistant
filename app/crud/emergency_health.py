...
async def get_emergency_health(db: AsyncSession, user_id: int):
    """
    Retrieves Emergency Health ID information for a user.
    """
    result = await db.execute(select(EmergencyHealthID).filter(EmergencyHealthID.user_id == user_id))
    return result.scalars().first()

async def create_emergency_alert(db: AsyncSession, user_id: int, source: str = "voice_command"):
    # Dummy implementation for emergency alert creation
    # In a real system, this would create an alert in the database
    return {"user_id": user_id, "source": source, "status": "alert_sent"}
