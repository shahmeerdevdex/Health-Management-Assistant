...
async def list_support_groups(db: AsyncSession = Depends(get_db),current_user=Depends(get_current_user)):
    result = await db.execute(select(SupportGroup))
    return result.scalars().all()


@router.get("/support-groups/{group_id}", response_model=SupportGroupResponse)
async def get_group(group_id: int, db: AsyncSession = Depends(get_db),current_user=Depends(get_current_user)):
...
async def create_support_group(group: SupportGroupCreate, db: AsyncSession = Depends(get_db),current_user=Depends(get_current_user)):
...
async def list_group_posts(group_id: int, db: AsyncSession = Depends(get_db),current_user=Depends(get_current_user)):
